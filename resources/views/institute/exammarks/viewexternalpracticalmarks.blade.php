                ...
                <div class="col-sm-12 well well-sm white-background minus15px-margin-top">
                    <div class="col-sm-12">
                        <button class="btn btn-primary btn-sm pull-right" onclick="window.print();return false;">
                            <span class="glyphicon glyphicon-print"></span> Print External Practical Marks
                        </button>
                    </div>
                </div>
            </div>
                                    ...
                                    @foreach($subjects as $s)
                                        @if($s->syear == $i)
                                            <th class="center-text">
                                                <u>Min</u><br> <span class="center-text"> {{ $s->emin_marks }} </span>
                                            </th>
                                            <th class="center-text">
                                                <u>Max</u><br> {{ $s->emax_marks }}
                                            </th>
                                        @endif
                                    @endforeach
                                </tr>
                                                ...
                                                @if($s->syear == $i)
                                                    <td colspan="2" class="center-text">
                                                        ...
                                                        @if(!is_null($app))
                                                            ...
                                                            @if(!is_null($mark))
                                                                @if(empty($mark->external))
                                                                    <span class="red-text">
                                                                No Exernal Marks Entered
                                                                </span>
                                                                @else
                                                                    <span @if($mark->external < $s->emin_marks || $mark->external == 'Abs') class="red-text" @php $failcount++ @endphp @else class="green-text" @endif>
                                                                        {{ $mark->external }}
                                                                    </span>

                                                                    @if($mark->external != 'Abs')
                                                                        @php $count += $mark->external;  @endphp
                                                                    @endif
                                                                @endif
                                                                    {{--
                                                                @if(!is_null($mark->external) || $mark->external != '' || empty($mark->external))
                                                                    <span @if($mark->external < $s->emin_marks || $mark->external == 'Abs') class="red-text" @php $failcount++ @endphp @else class="green-text" @endif>
                                                                        {{ $mark->external }}
                                                                    </span>
