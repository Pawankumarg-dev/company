...
@section('content')
    ...
    <form class="form-horizontal" role="form" method="POST"
          action="{{ url('/institute/exammarksentry/update-internal-practical-marks/') }}" autocomplete="off" accept-charset="UTF-8"
                    ...
                    <div class="row">
                        <div class="page-break">
                            <div class="col-sm-12 well well-sm white-background minus15px-margin-top">
                                                            ...
                                                            <td class="center-text">
                                                                ...
                                                                @if(!is_null($application))
                                                                    ...
                                                                    @if(is_null($mark))
                                                                        ...
                                                                        <input type="hidden" id="min_mark{{ $count }}" value="{{ $application->subject->imin_marks }}" />
                                                                        <input type="hidden" id="max_mark{{ $count }}" value="{{ $application->subject->imax_marks }}" />

                                                                        <input type="text" size="1" id="internal{{ $count }}" name="internal[]"
                                ...
                                @if($entrycount > 0)
                                    <input type="hidden" id="internal_fileterm{{ $j }}" name="internal_fileterm[]" value="{{ $i }}" />
                                    <div class="form-group">
                                        <label for="internal_file{{ $filecount }}" class="control-label col-sm-5">
                                            <div  class="text-left">
                                                <span class="blue-text">
                                                    Upload Scanned File of Internal Marks
                                                        <span class="red-text">
                                                        *
                                                    </span><br>
                                                </span>
                                        ...
                                        <div class="col-sm-3">
                                            <div class="col-sm-6">
                                                <input type="file" id="internal_file{{ $filecount }}" name="internal_file[]" onchange="validateFile({{ $filecount }})">
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    function markAbsent(count) {
        var internal = document.getElementById('internal'+count);
    ...
    function removeAbsent(count) {
        ...
        swal({
            ...
            if (result.value) {
                ...
                internal.removeAttribute('readOnly');
                internal.value = '';
                checkMarkValue(count);
    ...
    function checkMarkValue(count) {
        ...
        if(internal.value == 'Abs') {
            internal.style.backgroundColor = 'yellow';
            internal.style.color = 'red';
            internal.style.fontWeight = 'bold';
            internal.setAttribute('readOnly', true);
        }
        else if(isNaN(internal.value)) {
            internal.focus();
            internal.value = '';
            swal("Error Occurred!!!", "Please enter a Valid Mark.", "error");
        }
        else if(Number.isInteger(+internal.value) == false) {
            internal.focus();
            internal.value = '';
            swal("Error Occurred!!!", "Please enter a Valid Mark.", "error");
        }
            ...
            if (internal.value == '') {
                ...
                if (Number(internal.value) < Number(min_mark.value)) {
                    internal.style.backgroundColor = 'red';
                    internal.style.color = 'white';
                    internal.style.fontWeight = 'bold';
                }
                if (Number(internal.value) >= Number(min_mark.value)) {
                    internal.style.backgroundColor = 'green';
                    internal.style.color = 'white';
                    internal.style.fontWeight = 'bold';
                }
                if (Number(internal.value > Number(max_mark.value))) {
                    internal.focus();internal.value = '';
    ...
    function validateFile(count) {
        var ext = $("#internal_file"+count).val().split('.').pop().toLowerCase();
        if ($.inArray(ext, ['pdf']) == -1){
            swal("Error Occurred!!!", "Please upload the scanned file in .pdf format only.", "error");
            $('#internal_file'+count).val(null);
            return false;
        }
        else if ($("#internal_file"+count)[0].files[0].size > 3145728) {
    ...
    function validateForm() {
        ...
        for (var i = 0; i < "{{ $count }}"; i++) {
            var internal = document.getElementById('internal' + i);
        ...
        for (var i = 0; i < "{{ $filecount }}"; i++) {
            if($('#internal_file' + i).val() == '') {
                swal("Error Occurred!!!", "Please upload the Scanned file of Internal Marks.", "error");
                return false;
            }
        }
    }
