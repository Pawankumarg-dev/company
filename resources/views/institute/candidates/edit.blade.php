...
@section('style')
	...
	<script>
		...
		//	$('#nationality_id').val(86).change();
			...
			$('#fathername').attr('autofocus',true);
			@endif
			$('#name').attr('disabled',true);
			$('#enrolmentno').attr('disabled',true);
			...
			<div class="row">
				<div class="col-md-12">
					<ul class="breadcrumb" >
						...
						<li>{{$programme->course_name}}  ({{\App\Academicyear::where('current',1)->first()->year }}) </li>
			...
			<div class="row">
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						{{ Form::bsTextarea('comment',"Reason to edit the data *") }}	
					</div>
				</div>
					<div class="panel panel-default">
						<div class="panel-heading">
						...
						<div class="panel-body personal">
							...
							{{ Form::bsText('enrolmentno',"Enrolment No *",$candidate->enrolmentno,['class' => 'form-control disable']) }}
					...
					<div class="panel panel-default">
						...
						<div class="panel-body">
							<table class="table table-bordered">
								...
								@endif {{--
								@if(count($candidate->educations) > 2 )
								<tr>
									<td>
										<input type="text" name="edugrade_3" id="edugrade_3" placeholder="Any Other" class="form-control" />
									</td>
									<td>
										<input type="text" class="form-control" id="board_3" name="board_3"  value="@if($candidate->eductions->count() > 2){{$candidate->educations->whereNotIn('edugrade',['10th','12th'])->first()->board}}@endif"  >
									</td>
									<td  style="width:80px;">
										<input type="text"  class="form-control year" id="yop_3" name="yop_3"  value="@if($candidate->eductions->count() > 2){{$candidate->educations->whereNotIn('edugrade',['10th','12th'])->first()->yop}}@endif"  />
									</td>
									<td>
										<input type="text" class="form-control num" id="tmarks_3" name="tmarks_3"  value="@if($candidate->eductions->count() > 2){{$candidate->educations->whereNotIn('edugrade',['10th','12th'])->first()->tmarks}}@endif" >
									</td>
									<td>
										<input type="text" class="form-control num" id="omarks_3" onkeyup="javascript:calper(3);" name="omarks_3"  value="@if($candidate->eductions->count() > 2){{$candidate->educations->whereNotIn('edugrade',['10th','12th'])->first()->omarks}}@endif" >
									</td>
									<td>
										<input type="text" class="form-control" id="percentage_3" name="percentage_3"  value="@if($candidate->eductions->count() > 2){{$candidate->educations->whereNotIn('edugrade',['10th','12th'])->first()->percentage}}@endif"   >
									</td>
									<td>
										<input type="text" class="form-control" id="subjects_3" name="subjects_3"  value="@if($candidate->eductions->count() > 2){{$candidate->educations->whereNotIn('edugrade',['10th','12th'])->first()->subjects}}@endif" >
									</td>
								</tr>
								@endif
							--}}
				...
				<div class="col-md-12">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							{{ Form::bsFile('signature',"Signature *",'image',$candidate->signature,'files/enrolment/signature') }}
							...
							{{ Form::bsFile('doc_tenth', "10th Marksheet *",null,$candidate->doc_tenth,'files/enrolment/marksheets/tenth') }}

							<small>Only PDF file with size 200KB to 2048 KB allowed</small>
							@if($programme->programmegroup->twelveth == 1)
							{{ Form::bsFile('doc_twelveth', "12th Marksheet *",null,$candidate->doc_twelveth,'files/enrolment/marksheets/twelveth') }}
							<small>Only PDF file with size 200KB to 2048 KB allowed</small>

							@endif
							{{ Form::bsFile('doc_application', "Upload the scanned copy of the application form submitted by the student *",null,$candidate->doc_application,'files/enrolment/applications') }}
...
var Verhoeff = {
	...
	//		formData.append('dob','1955-01-01');
		...
		$(document).ready(function() {
			...
			console.log('T:' + $('input[name="community_id"]:checked').val());
            if($('input[name="community_id"]:checked').val()<2){
				console.log('c<2');
                $('#doc_community_form').addClass('hidden');
            }else{
                $('#doc_community_form').removeClass('hidden');
...
<script>
	...
	<?php if(!is_null($candidate->signature)): ?>
         //   $(".btn_signature").prop("disabled", true);
    <?php endif; ?>
