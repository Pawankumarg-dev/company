...
//			$('input[name="fathername"]').attr('readonly',true);
//			if($('input[name="mothername"]').val()!=''){
//				$('input[name="mothername"]').attr('readonly',true);
//			}
			@if($candidate->approvedprogramme->academicyear_id==9)
			//$('.btn_photo').attr('readonly',true);
			@endif
			@if(!strpos($candidate->email,'@email.com'))
		//	$('input[name="email"]').attr('readonly',true);
	//		$('input[name="contactnumber"]').attr('readonly',true);
			@endif
...
<form action="{{url('candidate/update'.'/'.$candidate->id)}}" enctype="multipart/form-data" method="post">
		...
		<div class="container" >
			...
			@if($candidate->incomplete_2024_data == 1)
				<div
					class="alert alert-danger"
				>
					Some of the condidate details are incomplete. Please check
					<ul>
						@if($candidate->dob == '0000-00-00') <li>DOB</li> @endif
						@if(is_null($candidate->state_id) || is_null($candidate->district_id) ||  is_null($candidate->pstate_id) ||  is_null($candidate->pdistrict_id) ) <li>Address</li> @endif
				...
				<div class="panel panel-default">
					...
					<div class="panel panel-default">
						...
						{{--	{{ Form::bsText('enrolmentno',"Enrolment Number*",$candidate->enrolmentno,['class' => 'form-control']) }} --}}
							...
							{{ Form::bsText('name',"Student's Name *",$candidate->name,['class' => 'form-control max99 ']) }}
						...
						<!--	@if($programme->programmegroup->id != '3')
							...
							<div class="form-group">
								...
								<input type="date" id="dob" name="dob" class="form-control" value="{{\Carbon\Carbon::parse($candidate->dob)->format('Y-m-d')}}" >
							</div>
							<!-- @if($candidate->incomplete_data == 1 ) -->
							<!-- <input type="text" id="dob" name="dob" readonly class="form-control" value="{{\Carbon\Carbon::parse($candidate->dob)->format('d-m-Y')}}" > -->
								<!-- @endif -->
							...
							<div class="form-group">
								...
								<input type="text" id="contactnumber" name="contactnumber" class="form-control" maxlength="10" value="{{$candidate->contactnumber}}" >
							</div>	
						...
						<div class="panel-body">
							<table class="table table-bordered">
								...
								<tr>
									...
									<td>
										<?php $tenth =  $candidate->educations()->where('edugrade','10th')->count() > 0 ? $candidate->educations()->where('edugrade','10th')->first() : null; ?>		
										<input type="text" class="form-control" id="board_1" name="board_1" value="@if(!is_null($tenth)){{$tenth->board}}@endif" />
									</td>
									<td  style="width:80px;">
										<input type="text"  class="form-control year" id="yop_1" name="yop_1" 
											value="@if(!is_null($tenth)){{$tenth->yop}}@endif"
										/>
									</td>
									<td>
										<input type="text" class="form-control num year" id="tmarks_1" name="tmarks_1" 
											value="@if(!is_null($tenth)){{$tenth->tmarks}}@endif"
										/>
									</td>
									<td>
										<input type="text" class="form-control num year" id="omarks_1" onkeyup="javascript:calper(1);" name="omarks_1"  
											value="@if(!is_null($tenth)){{$tenth->omarks}}@endif"
										/>
									</td>
									<td>
										<input type="text" class="form-control" id="percentage_1" name="percentage_1"  
											value="@if(!is_null($tenth)){{$tenth->percentage}}@endif"
										/>
									</td>
									<td>
										<input type="text" class="form-control" id="subjects_1" name="subjects_1"  
											value="@if(!is_null($tenth)){{$tenth->subjects}}@endif"
										/>
									</td>
								</tr>
								@endif
								...
								<td>
									...
									<td>
										<?php $twelveth =  $candidate->educations()->where('edugrade','12th')->count() > 0 ? $candidate->educations()->where('edugrade','12th')->first() : null; ?>		
										<input type="text" class="form-control" id="board_2" name="board_2"  
											value="@if(!is_null($twelveth)){{$twelveth->board}}@endif"
										/>
									</td>
									<td  style="width:80px;">
										<input type="text"  class="form-control year" id="yop_2" name="yop_2"  
										value="@if(!is_null($twelveth)){{$twelveth->yop}}@endif"
									/>
									</td>
									<td>
										<input type="text" class="form-control num" id="tmarks_2" name="tmarks_2"  
											value="@if(!is_null($twelveth)){{$twelveth->tmarks}}@endif"
										/>
									</td>
									<td>
										<input type="text" class="form-control num" id="omarks_2" onkeyup="javascript:calper(2);" name="omarks_2"  
											value="@if(!is_null($twelveth)){{$twelveth->omarks}}@endif"
										/>
									</td>
									<td>
										<input type="text" class="form-control" id="percentage_2" name="percentage_2"  
											value="@if(!is_null($twelveth)){{$twelveth->percentage}}@endif"
										/>
									</td>
									<td>
										<input type="text" class="form-control" id="subjects_2" name="subjects_2"  
											value="@if(!is_null($twelveth)){{$twelveth->subjects}}@endif"
										/>
									</td>
								</tr>
								@endif 
				...
				<div class="col-md-12">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							...
							{{ Form::bsFile3('signature',"Signature *",'image',$candidate->signature,'files/enrolment/signature') }}
							<small>
...
var Verhoeff = {
	...
	//		formData.append('dob','1955-01-01');
		...
		$(document).ready(function() {
			...
			var cid = $('input[name="community_id"]:checked').val();
            if(cid<2){
				console.log('c<2');
                $('#doc_community_form').addClass('hidden');
            }else{
