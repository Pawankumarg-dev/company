            ...
            <td>
                ...
                <table border="0" cellpadding="5" cellspacing="0" width="100%">
                    <tr>
                        <td colspan="2" class="center-text bold-text">
                            ENROLMENT PAYMENT {{ $enrolmentpayment->enrolmentfee->academicyear->year }} - ACKNOWLEDGEMENT RECEIPT
                        </td>
                    </tr>
                    <tr>
                        <td width="75%" class="right-text">Payment Reference No</td>
                        <td class="bold-text">: {{ $enrolmentpayment->reference_number }} </td>
                    </tr>
                </table>
                <table border="0" cellpadding="5" cellspacing="0" width="100%">
                    <tr>
                ...
                <table border="0" cellpadding="5" cellspacing="0" width="100%">
                    ...
                    <tr>
                        ...
                        <td class="bold-text">: {{ $enrolmentpayment->payment_date->format('d.m.Y') }}</td>
                        <td>Payment Mode</td>
                        <td class="bold-text">: {{ $enrolmentpayment->paymenttype->course_name }}</td>
                    </tr>
                    <tr>
                        <td>Payment Bank</td>
                        <td class="bold-text" colspan="3">: {{ $enrolmentpayment->paymentbank->bankname }}</td>
                    </tr>
                    <tr>
                        <td>Transaction ID / Number</td>
                        <td class="bold-text">: {{ $enrolmentpayment->payment_number }}</td>
                        <td>Total No. of Students</td>
                        <td class="bold-text">: {{ $enrolmentpayment->count() }} (list attached)</td>
                    </tr>
                    <tr>
                        <td>Amount Paid</td>
                        <td class="bold-text">: Rs. {{ $enrolmentpayment->amount_paid }} /- (@php echo no_to_words($enrolmentpayment->amount_paid) @endphp only)</td>
                    </tr>
                    <tr>
                        <td>Late fee Applicable</td>
                        <td class="bold-text">: {{ $enrolmentpayment->latefee_remark }}</td>
                    </tr>
                    <tr>
                        <td>Payment Remarks</td>
                        <td class="bold-text">: {{ $enrolmentpayment->payment_remark }}</td>
                    </tr>
                </table>
                <table border="0" cellpadding="5" cellspacing="0" width="100%">
                    <tr>
                ...
                <table border="0" cellpadding="5" cellspacing="0" width="100%">
                    ...
                    <tr>
                        <td colspan="2">
                            <b>{{ $institute->code }}</b> has remitted Rs. <b>{{ $enrolmentpayment->amount_paid }}</b> /-
                            (<b>@php echo no_to_words($enrolmentpayment->amount_paid) @endphp only</b>) towards the payment of Affiliation fee for
                            batch <b> {{ $enrolmentpayment->incidentalfee->academicyear->year }} ({{ $enrolmentpayment->incidentalfee->term }} year) - {{ $enrolmentpayment->incidentalfee->programme->course_name }} </b>
                            paid on <b>{{ $enrolmentpayment->payment_date->format('d-m-Y') }}</b>
                            <b>{{ $institute->code }}</b> - <b>{{ $institute->name }}</b> is remitted Rs. <b>{{ $enrolmentpayment->amount_paid }}</b> /-
                            (<b>@php echo no_to_words($enrolmentpayment->amount_paid) @endphp</b> only) on the Enrolment of <b>{{ $enrolmentpayment->count() }}</b>
                            students on <b>{{ $enrolmentpayment->payment_date->format('d-m-Y') }}</b>
                        </td>
                    </tr>
                </table>
                <br><br>
