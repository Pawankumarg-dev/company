...
<div class="container">
	<div class="row">
		<div class="col-12">
            ...
            <form action="{{url('institute/markentry/internal/save')}}" method="post">
                {{csrf_field()}}
                    ...
                    @foreach($applications->sortBy('candidate.enrolmentno') as $a)
                        <tr>
                            ...
                            <td>
                                ...
                                <input class="attn"  data-id="{{$a->id}}"  type="radio" name="attendence_{{$a->id}}" value="1"  @if($a->internalattendance_id == 1) checked @endif /> Present 
                                <input  class="attn" data-id="{{$a->id}}" type="radio" name="attendence_{{$a->id}}" value="2" @if($a->internalattendance_id == 2) checked @endif />  Absent 
                            </td>
                                <td>
                                    <input type="number" id="internal_{{$a->id}}" name="internal_{{$a->id}}"  value="{{$a->internal_mark}}" min="0"  max="{{$a->subject->imax_marks}}" disabled> 
                                </td>
                        </tr>
                    @endforeach
                </table>
