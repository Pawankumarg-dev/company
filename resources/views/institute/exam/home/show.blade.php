                    ...
                    <table class="table table-bordered table-condensed table-hover">
                        <thead>
                            <tr>
                                ...
                                <th rowspan="1" colspan="1" style="display:none;" class="center-text  ">External Marks
                                </th>
                        ...
                        <tbody>
                            ...
                            @foreach ($approvedprogrammes as $approvedprogramme)
                                ...
                                <tr class="{{ $class }}">
                                    ...
                                    $record = \App\Internalmarksheet::where('approvedprogramme_id', $approvedprogramme->id)->where('exam_id', $exam->id)->first();
                                   
                                    if (($record && $record->verified == 2 && $date < '2026-05-26') || ($approvedprogramme->institute_id == 987 && $date < '2026-05-26')) {
                                        $kk = 1;
                                    }
                                    ?>
                                    ...
                                    @endif --}}
                                        ...
                                        @if ($exam->id == 28 && $approvedprogramme->academicyear_id <= 16 && ($kk == 1))
                                            <a href="{{ url('attendanceupload/' . $approvedprogramme->id) . '/' . $exam->id . '/1' }}"
                                                class="btn btn-primary btn-xs"><i class="fa fa-th"></i> &nbsp;&nbsp;Term
                                                1</a>
                                            @if ($approvedprogramme->programme->numberofterms == 2 && $approvedprogramme->academicyear_id < 12)
                                                <a href="{{ url('attendanceupload/' . $approvedprogramme->id) . '/' . $exam->id . '/2' }}"
                                                    class="btn btn-primary btn-xs"><i class="fa fa-th"></i> &nbsp;&nbsp;Term
                                                    2</a>
                                            @endif
                                        @endif
                                    ...
                                    <td class="hidden">
                                        @if (
                                            $exam->id == 26 &&
                                                ($approvedprogramme->programme_id == 57 ||
                                                    ($approvedprogramme->id == 8797 && $approvedprogramme->institute_id == 1031)))
                                            <a href="{{ url('institute/exam/practicalmarkentry') }}/{{ $approvedprogramme->id }}?exam_id={{ $exam->id }}&subjecttype_id=2&syear=1"
                                    ...
                                    <td class="center-text">
                                        <a href="{{ url('institute/exam/applicants?exam_id=') }}{{ $exam->id }}&approvedprogramme_id={{ $approvedprogramme->id }}"
                                            class="btn btn-primary btn-sm " style="margin-left:10px;">
                                            <i class="fa fa-clone"></i> &nbsp;&nbsp;
                                            @if ($exam->publish_result == 1 && $exam->id < 28)
                                                Result
                                            @else
                                                Exam Applications
                                            @endif
