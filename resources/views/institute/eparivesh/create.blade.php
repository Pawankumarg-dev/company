				...
				<div class="col-md-12">
					<div class="panel panel-default">
						...
						<div class="panel-body personal">
							...
							<input name='epariveshreg' class="reg" value='{{$student->RegistrationNo}}' type='hidden' />

							{{-- {{ Form::bsText('epariveshreg', "RegistrationNo *", $student->RegistrationNo, ['class' => 'form-control reg']) }} --}}
							...
							<div class="form-group">
								...
								<input type="text" name='name' class="form-control" value="{{$student->FirstName . ' ' . $student->LastName}}" >
							</div>
							
							{{-- {{ Form::bsText('name', "Student's Name *", $student->FirstName . '' . $student->LastName, ['class' => 'form-control max99 p']) }} --}}
							{{ Form::bsText('fathername',"Father's Name *",$student->FatherName,['class' => 'form-control max99']) }}

							<!--{{ Form::bsSelectandText('salutation_id',$salutations,'salutation','Choose','fathername',"Father's / Husband's name") }}
							-->
							{{ Form::bsText('mothername',"Mother's name *",$student->MotherName,['class' => 'form-control max99']) }}
						<!--	@if($programme->programmegroup->id != '3')
							...
							@endif -->
														{{ Form::bsRadio('gender_id',$genders,'gender','Gender *',null,$student->gendername) }}

							{{-- {{ Form::bsRadio('isdisabled',$yesno,'value','PwD *') }} --}}
														{{ Form::bsRadio('isdisabled',$yesno,'value','PwD *',null,$student->IsPWD) }}

							<div class="showhideudid hidden">
								{{ Form::bsText('udid',"UDID or UDID Enrolment No",$student->PWDUDIDNo) }}
							...
							<div class="form-group">
								...
								<input type="date" id="dob" name="dob"  class="form-control" value="{{\Carbon\Carbon::parse($student->DateofBirth)->format('Y-m-d')}}" >
							</div>


							{{-- {{ Form::bsDate('dob',"Date of Birth *") }} --}}
							{{ Form::bsSelect('nationality_id',$nationalities,'name','Nationality *',null,$student->nationality_id)}}

							<div class="form-group">
								<label for="aadhar" class="control-label">
										Aadhar Number *
								</label>
								<input type="text" id="aadhar" name="aadhar" required class="form-control" value="{{ old('aadhar') }}" >
							</div>
							
							{{ Form::bsRadio('community_id',$communities,'community','Category *',null,$student->communities_id) }}

							{{ Form::bsRadio('ews',$yesno,'value',"Do you belong to EWS category? *"),$student->IsEWS }}
							{{ Form::bsEmail('email',"Email ID *"),$student->email }}
							{{-- Laravel Form Input --}}
<div class="form-group">
	    <label for="Mobile Number *" class="control-label">Mobile Number *</label>
        <input class="form-control phone" maxlength="10" name="contactnumber" type="text" value="">
				<small class="text">Mobile No should be unique</small>
...
<label for="otpInput">Enter OTP:</label>
  <input type="text" id="otpInput" maxlength="6" class="form-control" name="ottp" placeholder="OTP" required/>
...
<script>
  ...
  // Listen for input changes in the phone field
  phoneInput.addEventListener("input", () => {
    const phone = phoneInput.value;
let RegistrationNo = document.querySelector(".reg").value;
    if (phone.length === 10 && /^\d{10}$/.test(phone)) {
      ...
      fetch('/eotp', {
        ...
        body: JSON.stringify({ phone: phone,RegistrationNo:RegistrationNo })
      })
      .then(response => response.json())
      .then(data => {
        if (data.success) {
          generatedOTP = data.otp; // For demo/testing only
          otpSection.style.display = "block";
          message.textContent = "OTP sent to your number.";
          otpInput.value = "";
          console.log("OTP (from server):", generatedOTP);
        } else {
      ...
      .catch(error => {
        ...
        message.textContent = "An error occurred while sending OTP.";
      });
    } else {
				...
				<div class="col-md-6">
					<div class="panel panel-default">
						...
						<div class="panel-body">
							{{ Form::bsText('address',"Addresss *",$student->CorrespondanceAddress) }}
									{{ Form::bsSelect('state_id',$states,'state_name','State *',null,$student->cstate_id)}}
				...
				<div class="col-md-6">
					<div class="panel panel-default">
						...
						<div class="panel-body per-add">
							{{ Form::bsText('paddress',"Addresss *",$student->PermanentAddress,['class'=>'form-control saa']) }}
							{{ Form::bsSelect('pstate_id',$states,'state_name','State *',null,$student->pstate_id)}}
						...
						<div class="panel-body">
							<table class="table table-bordered">
								...
								<tr>
									...
									<td>
										<input type="text" class="form-control" id="board_1" name="board_1" value="" />

										
									</td>
									<td  style="width:80px;">
										<input type="text"  class="form-control year" id="yop_1" name="yop_1" value="" />
									</td>
									<td>
										<input type="text" class="form-control num year" id="tmarks_1" name="tmarks_1" value="" />
									</td>
									<td>
										<input type="text" class="form-control num year" id="omarks_1" onkeyup="javascript:calper(1);" name="omarks_1"  value=""  >
									</td>
									<td>
										<input type="text" class="form-control" id="percentage_1" name="percentage_1"  value=""   >
									</td>
									<td>
										<input type="text" class="form-control" id="subjects_1" name="subjects_1"  value="" >
									</td>
								</tr>
								@endif
								...
								<td>
									...
									<td>
										<input type="text" class="form-control" id="board_2" name="board_2"  value=""  >
									</td>
									<td  style="width:80px;">
										<input type="text"  class="form-control year" id="yop_2" name="yop_2"  value="" />
									</td>
									<td>
										<input type="text" class="form-control num" id="tmarks_2" name="tmarks_2"  value=""  >
									</td>
									<td>
										<input type="text" class="form-control num" id="omarks_2" onkeyup="javascript:calper(2);" name="omarks_2"  value=""  >
									</td>
									<td>
										<input type="text" class="form-control" id="percentage_2" name="percentage_2"  value=""  >
									</td>
									<td>
										<input type="text" class="form-control" id="subjects_2" name="subjects_2"  value="" >
									</td>
								</tr>
								@endif
								<tr>
...
var Verhoeff = {
 ...
 $(document).ready(function () {
    ...
    if (selectedState) {
        console.log('Onload state:', selectedState);
        fetchDistricts(selectedState);
    }
});
 window.onload = function() {
        loadPDistricts({{ $student->cstate_id }});

		 if($('input[name="community_id"]:checked').val()>1){
                $('#doc_community_form').removeClass('hidden');
            }
    };
	</script>
