						...
						<td>
							...
							@if($c->status_id==2)
								@if(file_exists(public_path('files/enrolment/photos/'.$c->photo)))
									@if($c->applications->count()>0)
										@if($c->attendances->where('exam_id',$exam_id)->count()>0)
											@if($c->approvedprogramme->programme->programmegroup->hallticket_download==1)
												@if($c->attendances->where('exam_id',$exam_id)->first()->attendance_t >= 75)
													{{--<a href='/hallticket?id={{$c->id}}&exam_id={{$exam_id}}' class="btn btn-xs btn-info"> <i class="fa fa-download"></i> &nbsp;&nbsp; Hallticket</a> --}}
												@else

													@if($c->attendances->where('exam_id',$exam_id)->first()->exemption == '2')
													{{--	<a href='/hallticket?id={{$c->id}}&exam_id={{$exam_id}}' class="btn btn-xs btn-info"> <i class="fa fa-download"></i> &nbsp;&nbsp; Hallticket</a> --}}
													@else
														@if($c->attendances->where('exam_id',$exam_id)->first()->document_exemption != null)
															@if($c->attendances->where('exam_id',$exam_id)->first()->exemption == '1')
																<i class="fa fa-info-circle"></i> &nbsp; Pending approval for Exemption.
															@endif
															@if($c->attendances->where('exam_id',$exam_id)->first()->exemption == '3')
																<i class="fa fa-info-circle"></i> &nbsp; Request for Exemption is rejected.
															@endif
														@else
															@if($c->attendances->where('exam_id',$exam_id)->first()->attendance_t == null)
																<i class="fa fa-info-circle"></i> &nbsp;Attendance not uploaded.
															@else
								...
								@endif
							@else
								<i class="fa fa-info-circle"></i> &nbsp;Candidate documents not approved.
							@endif
						</td>
