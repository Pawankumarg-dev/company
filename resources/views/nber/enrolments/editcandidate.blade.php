...
@section('content')
    ...
    <form action="{{url('/enrolments/editform/candidate/'.$candidate->id)}}" enctype="multipart/form-data" method="post">
        {!! csrf_field() !!}
                ...
                <div class="col-md-6">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            ...
                            {{ Form::bsText('name','Name: (As to be printed on certificate)',$candidate->name) }}
                            {{ Form::bsSelect('gender_id',$genders,'gender','Gender',null,$candidate->gender_id) }}
                            {{ Form::bsSelect('community_id',$communities,'community','Community Category',null,$candidate->community_id) }}
                            {{ Form::bsSelect('disability_id',$disabilities,'disability','Disability Type',null,$candidate->disability_id) }}
                            {{ Form::bsSelectandText('salutation_id',$salutations,'salutation','Choose','fathername',"Father's / Husband's name",[null],$candidate->fathername,null,$candidate->salutation_id) }}
                            {{ Form::bsText('mothername',"Mother's name",$candidate->mothername) }}
                            @if($programme->programmegroup->id != '3')
                                {{ Form::bsNumber('percentage',"HSc Percentage",1,100,$candidate->percentage) }}

                                <h4 class="red-text">
                                    <input type="checkbox" name='pex' id="pex" @if($candidate->doc_percentage_exception) {{'checked'}} @endif> Please click here to Apply for Percentage Exception </input> <br />
                                </h4>
                            @endif
            ...
            $.ajax({
                ...
                error: function (data) {
                    ...
                    if (data.status === 422) {
                        ...
                        $('#myModal').modal('hide');
                        $("#photo_filename").prop('src',"{{asset('files/enrolment/photos/cropped/')}}/"+$('#photo').val());
