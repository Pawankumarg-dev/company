    ...
    <div class="container">
        <div class="row">
            <div class="col-sm-12">
                ...
                <h2>
                    Enrolment Fee - {{Session::get('academicyear')}} -  {{$i->name }} ( {{$i->rci_code}})
                </h2>
                    <?php $pcount = 0; ?>
                        ...
                        <table class="table table-bordered table-hover">
                            ...
                            <tr>
                                ...
                                <th class="text-right">
                                    <?php 
                                        $order_ids = \App\Enrolmentfeepayment::where('institute_id',$institute_id)->where('academicyear_id',Session::get('academicyear_id'))->where('nber_id',$nber_id)->pluck('order_id')->toArray(); 
                        ...
                        @foreach($enrolmentfees as $enrolmentfee)
                            @if($enrolmentfee->nber_id == $nber_id)
                                                ...
                                                @foreach($enrolmentfee->orders as $order)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            @endif
                                                            @if($order->order_status != "Success")
                                                                    <a href="{{url('/institute/enrolmentfee/recheck/')}}/{{$order->id}}/{{$enrolmentfee->id}}">Refresh</a>
                                                            @else
                                                            <a style="display:none;" href="{{url('/institute/enrolmentfee/recheck/')}}/{{$order->id}}/{{$enrolmentfee->id}}">Refresh</a>
                                                            @endif
                                                        </td>
                                                    </tr>
                                                @endforeach
