...
?>
    ...
    <td>
            ...
            @if(!is_null($qp))
                <form method="post"  enctype="multipart/form-data" action="{{url('nber/updatequestionpaperupload')}}/{{$timetable->id}}/{{$language->id}}">
                    {{ csrf_field() }}
                    <input type="file" name="file" >  </input>
                    <button type="submit" class="btn btn-xs btn-primary">Re-Upload</button>
                </form>
            @else
                <form  method="post"  enctype="multipart/form-data" action="{{url('nber/questionpaperupload')}}/{{$timetable->id}}/{{$language->id}}">
                    {{ csrf_field() }}
