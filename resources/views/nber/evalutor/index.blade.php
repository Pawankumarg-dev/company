...
@section('content')
	<div class="container">
		<div class="row">
			<div class="col-sm-12">
				...
				<a href="{{url('/nber/evaluators/create')}}" style="position: absolute;right:15px;top:10px;" class="btn  mb-2 btn-primary btn-xs pull-right">Add New</a>
					...
					@foreach($evaluator as $e)
						<tr>
							...
							<td>
                                ...
                                <a href="{{url('nber/evaluators/')}}/{{$e->id}}" class="btn btn-xs btn-secondary">Edit</a>
                                @endif
                                @if($e->is_verified ==1)
                                <button class="btn btn-xs btn-danger deleteButton">Verified</button>

                                <button class="btn btn-xs btn-danger sendpasswordButton" data-id="{{$e->id}}">Sendpassword</button>
    ...
    $('.sendpasswordButton').on('click', function() {
                ...
                if (confirm('Are you sure you want to send the password to this evaluator?')) {
                    ...
                    $.ajax({
                        url: '/nber/evaluators/send-password', 
                        type: 'POST',     
                        data: {
                            _token: '{{ csrf_token() }}',  
                            item_id: itemId               
