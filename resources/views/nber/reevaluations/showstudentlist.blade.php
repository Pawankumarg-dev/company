    ...
    <div class="container">
        <div class="row">
            <div class="col-sm-12">
                ...
                <form action="{{ url('nber/exams/reevaluation') }}" method="get" class="form-group float-right"
                    style="float:right;width:400px;margin-left:5px;">
                    {{ csrf_field() }}
                    <div class="input-group">
                        <select name="evaluationcenter_id" id="evaluationcenter_id" class="form-control">
                            ...
                            @foreach ($evaluationcenters as $ec)
                                <option value="{{ $ec->id }}" @if (!is_null($evaluation_center) && $ec->id == $evaluation_center->id) selected @endif>
                                    ({{ $ec->code }})-{{ $ec->name }}</option>
                            @endforeach
                        </select>
                        ...
                        <span class="input-group-btn">
                            <button type="submit" class="btn btn-sm btn-primary" style="padding:24px 10px!important;"> Go
                            </button>
                        </span>
                    </div>
                </form>
                ...
                <form action="{{ url('nber/exams/reevaluation') }}" method="get" class="form-group float-right"
                    style="float:right;width:300px;margin-right:5px;">
                    {{ csrf_field() }}
                    <div class="input-group">
                        <input type="text" name="searchsubjectapplication" class="form-control"
                            placeholder="Dummy Application ID (From CRC)">
                        <span class="input-group-btn">
                            <button type="submit" class="btn btn-sm btn-primary"> Search </button>
                        </span>
                    </div>
            ...
            <div class="col-sm-12">
                <div class="table-responsive">
                    <button id="btnExport" style="margin-left:10px;"
                        class="btn hidden btn-primary btn-xs pull-right">DOWNLOAD</button>
                    <table class="table table-bordered table-condensed" id="myTable">
                        ...
                        @if ($reevaluationapplications)
                            ...
                            @foreach ($reevaluationapplications->sortBy('id') as $reevaluationapplication)
                                <tr>
                                    <td class="center-text">{{ str_pad($sno, 3, '000', STR_PAD_LEFT) }}@php $sno++; @endphp
                                    </td>
                                    ...
                                    <td class="center-text">
                                        {{ $reevaluationapplication->candidate->approvedprogramme->institute->rci_code }}
                                    </td>
                                    <td>{{ $reevaluationapplication->candidate->approvedprogramme->programme->course_name }}
                                    </td>
                                        ...
                                        <ul style="margin: 0; padding-left: 15px; list-style-type: disc;">
                                            @foreach ($reevaluationapplication->reevaluationapplicationsubjects as $subjectRelation)
                                                <li style="line-height: 1.3;">
                                                    ...
                                                    @php
                                                        $statuses = [];
                                                        if ($subjectRelation->retotalling_applystatus) {
                                                            $statuses[] = 'Retotalling';
                                                        }
                                                        if ($subjectRelation->reevaluation_applystatus) {
                                                            $statuses[] = 'Reevaluation';
                                                        }
                                                    @endphp
                                                    @if (count($statuses))
                                                        ({{ implode(' & ', $statuses) }})
                                    ...
                                    <td>
                                        ...
                                        @else
                                            Payment Pending
                                            <a href="{{ url('nber/recheckStatusall') }}/{{ $reevaluationapplication->id }}"
                                                class="btn btn-sm btn-primary" style="margin-top:3px;">Refresh Payment
                                                Status</a>
                                        @endif
                                    </td>
