...
<div class="form-group" id="{{$name}}_form">
    ...
    @else
     	@if($image==null)
    		<a target="_blank" id='filename' style='margin-left: 10px;' class="hidden ">  </a>
    	@else
    ...
    @if($image==null)
    <input name='file_{{$name}}_{{$cid}}' id="file_{{$name}}_{{$cid}}" class="file hidden" type="file"  /> 
    @else
   	<input name='file_{{$name}}_{{$cid}}' id="file_{{$name}}_{{$cid}}" class="file hidden" type="file"  accept="image/*" /> 
    @endif
    {{ Form::text($name,'',['id'=>$name,'class'=>'hidden']) }}
    &nbsp;&nbsp;
    <button  class="btn btn-xs btn-primary btn_{{$name}}"type="button" onclick="event.preventDefault();  document.getElementById('file_{{$name}}_{{$cid}}').click();">
		...
		<script>
	    $("#file_{{$name}}_{{$cid}}").on('change',function(){
	    	if(this.files[0].size > 2097152){
				...
				$.ajax({
				    url: '{{url("/institute/fileuploaddirect")}}',
