	    	...
	    	if(this.files[0].size > 2097152){
				...
				$.ajax({
				    ...
				    if (data.status === 422) {
				        ...
				        $('#file_{{$name}}').val('');
				        @if($image!=null)
				       		var fileurl = '{{url("files")}}/{{Auth::user()->database_name}}/temp/"'+fname;
							...
							$('#image').cropper('replace',"{{asset('files')}}/{{Auth::user()->database_name}}/temp/"+fname);
							$('#modalbtn').click();
						@else
							$("#{{$name}}_filename").prop('href',"{{asset('files')}}{{Auth::user()->database_name}}/temp/"+fname);
