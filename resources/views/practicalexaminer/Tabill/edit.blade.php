        ...
        <div class="row">
            ...
            <div class="form-group col-md-6">
                ...
                <select class="form-control" name="nber_id" id="nber_id" required>
                    ...
                    @foreach($nbers as $n)
                        <option @if($n->id==$tabill->nber_id) selected @endif value="{{ $n->id }}">{{ $n->name_code }}</option>
                    @endforeach
                </select>
            </div>
             <!-- NBER Selection -->
            <div class="form-group col-md-6">
                <label for="nber_id">Payment For {{$n->payment_for}}</label>
                <select class="form-control" name="payment_for" id="payment_for" required>
                    <option value="" selected disabled>Please select</option>
                        <option @if($tabill->payment_for=="Paractical Examiner") selected @endif value="Paractical Examiner">Paractical Examiner</option>
                        {{-- <option @if($tabill->payment_for=="Clo") selected @endif value="Clo">Clo</option>
                        <option @if($tabill->payment_for=="Evaluator") selected @endif value="Evaluator">Evaluator</option>
                        <option @if($tabill->payment_for=='Others') selected @endif value="Others">Others</option> --}}

                </select>
            </div>
            <div class="form-group col-md-6">
                <label for="consent_form">Report</label>
            ...
            <div class="form-group col-md-6">
                <iframe src="{{ url('/') }}/files/examcenter/Clo-report/{{$tabill->clo_report}}" frameborder="12"></iframe>
            </div>
            <div class="form-group col-md-6">
                <iframe src="{{ url('/') }}/files/examcenter/TABILL/{{$tabill->ta_form}}" frameborder="12"></iframe>
            </div>
            <div class="col-md-12">
