...
@section('content')
    ...
    <section>
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-12 well well-sm white-background minus15px-margin-top">
                    <div class="center-text bold-text blue-text">
                        {{$exam->name}} Exam - Question Paper Download Page
                    </div>
                </div>
            </div>
        </div>
    ...
    @else
        @foreach($startdates as $st)
            @php $sno = 1; @endphp
            @foreach($examtimetables as $et)
                @if($st->startdate == $et->startdate)
                    <section>
                        <div class="container-fluid">
                            <div class="row">
                                <div class="col-sm-12 well well-sm white-background minus15px-margin-top">
                                            ...
                                            <table class="table table-bordered table-condensed">
                                                <tr class="grey-background">
                                                    <th class="center-text" colspan="6">
                                                        {{ $et->startdate->format('d-m-Y h:i A') }} [{{ $et->startdate->format('l') }}]
                                                    </th>
                                                </tr>
                                                <tr class="grey-background">
                                                    ...
                                                    <th class="center-text" width="10%">Question Papers</th>
                                                    <th class="center-text" width="15%">Password</th>
                                                    ...
                                                    @if($et->enddate > \Carbon\Carbon::now())
                                                        @if($et->startdate < \Carbon\Carbon::now()->addMinutes(30))
                                                            <td class="center-text">
                                                                <a href="{{ url('/externalexamcenter/download-question-papers/'.$et->id) }}" class="btn btn-sm btn-success" target="_blank">Download</a>
                                                            </td>
                                                            <td class="center-text">
