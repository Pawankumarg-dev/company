...
class GenerateRevalutionMarksheet extends Job implements ShouldQueue
{
...
//        $this->supplimentaryService->generateMarksheet($cid);
    ...
    public function generatems($cid,$term){
        ...
        $barcode =  $d->getBarcodeHTML(url("marksheet").$aid.'/'.$rid.'/'.$term.'/re/25', 'QRCODE',2.5,2.5);
        $applicantid = str_pad($aid,5,'0',STR_PAD_LEFT);
        $file = '/var/www/html/rcinber/public/files/marksheet/re/25/'.$term.'_'.$rid.'_'.$applicantid.'.pdf';
        echo $file;
        ...
        $pdf = PDF::loadView('common.marksheet_new')->setPaper('a4','landscape');
        echo 'PDF Generated';
        $output = $pdf->output();
    ...
    public function generatecertificate($cid){
        ...
        $barcode =  $d->getBarcodeHTML(url('certificate/25/').$rid.'/'.$applicantid, 'QRCODE',2.5,2.5);
        view()->share('candidate',$candidate);
        view()->share('barcode',$barcode);
 	    $pdf = PDF::loadView('common.certificate_new')->setPaper('a4','portrait');
	    $output = $pdf->output();
        file_put_contents($file, $output);
        unset($pdf);
        unset($output);
