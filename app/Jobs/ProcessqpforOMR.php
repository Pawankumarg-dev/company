...
class ProcessqpforOMR extends Job implements ShouldQueue
{
    ...
    public function handle()
    {
     //   $timetables  = Examtimetable::where('examschedule_id',$this->schedule_id)->get();
       ...
       // $ttcount = $timetables->count();
        ...
        // echo "Step 2";
             $exampapers = Allexampaper::where('examschedule_id',$this->schedule_id)->groupBy('externalexamcenter_id')->where('omr_code',$this->omr_code)->groupBy('omr_code')->get();
                        ...
                        foreach($examtimetable->languages()->get() as $l){
                            ...
                            for($set=1;$set<4;$set++){
                                ...
                                if(!is_null($file)){
                                    ...
                                    $esid = $ep->examtimetable->examschedule_id;
                                    if($this->omr_code == 36112 && $esid == 69){
                                        $esid = 71;
                                    }
                                    $op = shell_exec('sh '. $watermark . ' ' . app_path(). ' ' . public_path(). ' '. $file. ' '. $set . ' ' . $ep->externalexamcenter_id . ' ' . $l->id . ' ' . $ep->examtimetable->subject->omr_code . ' ' . $esid);
                                }
                            }
                        }
                    }   
        ...
        $exampapers = Allexampaper::where('examschedule_id',$this->schedule_id)->groupBy('externalexamcenter_id')->where('omr_code')->groupBy('omr_code')->get();
        echo "Step 3" . PHP_EOL;
        $counter = 1;
        foreach($exampapers as $ep){
        //    if($ep->encrypted == 2){
                        ...
                        foreach($examtimetable->languages()->get() as $l){
                            for($set=1;$set<4;$set++){
                                ...
                                if(!is_null($file)){
                                    ...
                                    $op = shell_exec('sh '. $encrypt . ' ' . app_path(). ' ' . public_path(). ' '. $file. ' '. $set . ' ' . $ep->externalexamcenter_id. ' '. $password . ' ' . $l->id . ' ' . $ep->examtimetable->subject->omr_code . ' ' . $esid);                                
                                }
                            }
                        }
                    }
