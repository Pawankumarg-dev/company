...
class Extractallqp extends Job implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($exam_id,$course_id)
    {
    ...
    public function overlays(){
        ...
        foreach($examcenters as $ec){
            $this->generatePDF($ec->externalexamcenter_id);
        }
    }
    public function handle()
    {
        ...
        if($this->course_id == 0){
            ...
            foreach($courses as $c ){
                $this->processQP($c->id,78);
                $this->processQP($c->id,77);
            }
        }else{
            //$this->processQP($this->course_id);
        }
    }

    public function processQP($course_id,$ecid){
        $course = \App\Course::find($course_id);
        echo "STARTING FOR " .$course->name . " NBER ID". $course->nber_id . PHP_EOL;
        
        $timetables  = Examtimetable::where('exam_id',$this->exam_id)
                        ->where('examschedule_id',$ecid)
                        ->whereHas('subject',function($q) use($course_id){
                            $q->whereHas('programme', function($x) use($course_id){
                                $x->where('course_id',$course_id);
                            });
                        ...
                        foreach($tt->languages()->get() as $l){
                            for($set=1;$set<4;$set++){
                                ...
                                if(!is_null($file)){
                                    ...
                                    $op = shell_exec('sh '. $decrypt . ' ' . app_path(). ' ' . storage_path(). ' '. $file. ' '. $l->id . ' ' . $nipassword. ' ' . $tt->subject->omr_code . ' ' . $set . ' '. $tt->examschedule_id);
                                }
                            }
                        }
                        $tt->qp_process = 2;
