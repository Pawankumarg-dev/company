...
class ExtractQPNBER extends Job implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct($exam_id,$nber_id)
    {
    ...
    public function handle()
    {
        ...
        $timetables  = Examtimetable::where('exam_id',$this->exam_id)
                        ->whereHas('subject',function($q) use($nber_id){
                            $q->where('qp_prepared_by_nber_id',$nber_id);
                        ...
                        foreach($tt->languages()->get() as $l){
                            for($set=1;$set<4;$set++){
                                ...
                                if(!is_null($file)){
                                    ...
                                    $op = shell_exec('sh '. $decrypt . ' ' . app_path(). ' ' . public_path(). ' '. $file. ' '. $l->id . ' ' . $nipassword. ' ' . $tt->subject->omr_code . ' ' . $set);
                                }
                            }
                        }
                        $tt->qp_process = 2;
