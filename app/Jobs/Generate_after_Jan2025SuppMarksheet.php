...
class Generate_after_Jan2025SuppMarksheet extends Job implements ShouldQueue
{
...
//        $this->supplimentaryService->generateMarksheet($cid);
    ...
    public function generatems($cid,$term,$exam_id){
        ...
        $barcode =  $d->getBarcodeHTML(url("marksheet").$aid.'/'.$rid.'/'.$term.'/'.$exam_id.'', 'QRCODE',2.5,2.5);
        ...
        $pdf = PDF::loadView('common.marksheet_jan_2025')->setPaper('a4','landscape');
        echo 'PDF Generated';
        $output = $pdf->output();
    ...
    public function generatecertificate($cid,$exam_id){
        ...
        $file = '/var/www/html/rcinber/public/files/certificate/'.$exam_id.'/'.$rid.'_'.$applicantid.'.pdf';
        $barcode =  $d->getBarcodeHTML(url('certificate/'.$exam_id.'/').$rid.'/'.$applicantid, 'QRCODE',2.5,2.5);
        view()->share('candidate',$candidate);
        view()->share('barcode',$barcode);
