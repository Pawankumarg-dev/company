...
class Generate2024Marksheet extends Job implements ShouldQueue
{
...
/*            if($term==1 &&  $candidate->currentapplicant->current_version_term_one < $candidate->new_changes){
  ...
  */          $generate = true;
            ...
            $applications = Newapplication::where('candidate_id',$candidate->id)->where('term',$term)->get();
            if(1){
                if($applications->count() > 0){
                    $na = \App\Newapplicant::where('candidate_id',$candidate->id)->first();
                    $aid = $na->id;
                ...
                // if(!file_exists($file)){
                        ...
                        $barcode =  $d->getBarcodeHTML('https://nber-rehabcouncil.gov.in//marksheet/'.$aid.'/'.$rid.'/'.$term.'/25', 'QRCODE',2.5,2.5);
                        ...
                        $pdf = PDF::loadView('common.2024marksheet')->setPaper('a4', 'landscape');
                        $output = $pdf->output();
            ...
            //    $currentapplicant = \App\Newapplicant::find($candidate->newapplicant_id);
                if($term==1){
