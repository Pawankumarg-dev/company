...
class IncidentalchargeController extends Controller
{
    ...
    public function showDetails(Request $r) {
            ...
            //  //   $status = $this->recheckStatus($r);
            // }
            // if($status == 1){
            //     Session::put('messages','Payment Successfull');
            ...
            $billing_notes = $r->exam_name." exam fee payment for  - " . $candidate->enrolmentno;

            $applicant = \App\Allapplicant::where('candidate_id', $candidate->id)->where('exam_id', $exam_id)->first();
            Session::put('total',$applicant->amount);


            $order_number = "JE".$candidate->id."OR".date('Ymdhis').$this->generateRandomString().rand(1000, 9999);
            ...
            return view('student.examapplication.submit',compact('payment','type','institute','academicyear_id','billing_notes','billing_designation','billing_name','billing_tel','billing_email','nber_id','exam_id'));
        }
        ...
        if($r->type=='reevaluation'){
            ...
            $candidate = \App\Candidate::where('user_id',Auth::user()->id)->first();
            $billing_notes = "Reevaluation fee payment for July 2025 - " . $candidate->enrolmentno;
            if($r->has('reevaluationapplication_id')){
                ...
                $reevaluationapplication = Reevaluationapplication::where('exam_id',$exam_id)->where('candidate_id',$candidate->id);
            ...
            if($reevaluationapplication->count() == 0){
                $count = Reevaluationapplication::where('exam_id',$exam_id)->where('nber_id',$nber_id)->count();
                $count++;
                $nber  = \App\Nber::find($nber_id)->short_code;
                $applicationnumber = $exam_id.$nber.'REAPPN'.$this->generateRandomString().rand(1000, 9999).str_pad($count, 4, '0', STR_PAD_LEFT);
                ...
                foreach($subjects as $subject){
                    if($r->has('reevaluation_'.$subject->id) || $r->has('retotal_'.$subject->id) ||$r->has('photocopy_'.$subject->id) ){
                        $newapplication_id = \App\Allapplication::where('candidate_id',$candidate->id)->where('subject_id',$subject->id)->where('exam_id',$exam_id)->first()->id;
        ...
        if($r->type=='supplimentary2025'){
            ...
            $candidate = \App\Candidate::where('user_id',Auth::user()->id)->first();
            $billing_notes = "Supplimentary exam 2025 fee payment for  - " . $candidate->enrolmentno;
            $applicant = \App\Allapplicant::find($r->applicant_id);
            Session::put('total',$applicant->amount);
            $order_number = "exam".$candidate->id."OR".date('Ymdhis').$this->generateRandomString().rand(1000, 9999);
            Session::put('order_number',$order_number);
            ...
            $academicyear_id= $candidate->approvedprogramme->academicyear_id;
            $institute = Institute::find($candidate->approvedprogramme->institute_id);
            return view('student.examapplication.submit',compact('payment','type','institute','academicyear_id','billing_notes','billing_designation','billing_name','billing_tel','billing_email','nber_id'));
        }
        ...
        if($r->type=='june2025'){
            ...
            $candidate = \App\Candidate::where('user_id',Auth::user()->id)->first();
            $billing_notes = "June 2025 exam fee payment for  - " . $candidate->enrolmentno;
            $applicant = \App\Allapplicant::find($r->applicant_id);
            ...
            if(is_null($applicant->orders()->first())){
                ...
                $order_number = $applicant->orders()->first()->order_number;
            }
        ...
        if($r->type=='enrolment_student'){
            ...
            if(!is_null($paymentType)){
                $table = $paymentType->tablename;
                $row =  $table::find($r->id);
                if(!is_null($row)){
                    $table=$paymentType->tablename;
                    $billing_notes = $paymentType->billing_notes . $row->id;
                    Session::put('total',$r->amount);
                    $billing_name = $r->billing_name;
                    $billing_designation = $r->billing_designation;
                    $billing_tel = $r->billing_tel;
