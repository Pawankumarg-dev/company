...
class MarksheetController extends Controller
{
    ...
    public function recertificate($cid){
        ...
        if($currentapplicant->incomplete != 1 || ($currentapplicant->incomplete && $currentapplicant->candidate->isdisabled ==1) ){
             if( !is_null($currentapplicant) && Auth::user()->id == $currentapplicant->candidate->user_id ){
                 ...
                 if(file_exists($file)){
                   ...
                   //  $job = (new \App\Jobs\GenerateCertificate($currentapplicant->candidate_id))->onQueue('studentpc');
                    // $this->dispatch($job);
       ...
       if($currentapplicant->incomplete != 1 || ($currentapplicant->incomplete && $currentapplicant->candidate->isdisabled ==1) ){
            if( !is_null($currentapplicant) && Auth::user()->id == $currentapplicant->candidate->user_id ){
                ...
                if(file_exists($file)){
                  ...
                  //  $job = (new \App\Jobs\GenerateCertificate($currentapplicant->candidate_id))->onQueue('studentpc');
                   // $this->dispatch($job);
                    //Session::put('messages','Your certificate is being generated. Please wait and try again in a minute.');
                    Session::put('messages','Not Generated');
                    return back();
                }
            }
    ...
    public function downloadmsjan2025($exam_id,$cid,$term){
        ...
        return redirect(url('/files/marksheet')."/".$exam_id."/".$term.'_'.$rid.'_'.$applicantid.'.pdf');
        ...
        $rid = $sa->randstrig;
        $applicantid = str_pad($aid,5,'0',STR_PAD_LEFT);
            return redirect(url('/files/marksheet')."/".$exam_id."/RE_".$term.'_'.$rid.'_'.$applicantid.'.pdf');

    }

    public function downloadcertjan2025(Request $r, $exam_id,$cid){
        $sa = \App\Allapplicant::where('candidate_id',$cid)->where('exam_id',$exam_id)->first();
        $rid = $sa->randstrig;
        $applicantid = str_pad($sa->id,5,'0',STR_PAD_LEFT);

                     if($r->type=='re'){
                                return redirect(url('/files/certificate')."/".$exam_id.'/RE_'.$rid.'_'.$applicantid.'.pdf');

                     }

        return redirect(url('/files/certificate')."/".$exam_id.'/'.$rid.'_'.$applicantid.'.pdf');
    }
    ...
    public function downloadmsjuly2024($cid,$term){
    ...
    public function downloadcertjuly2024($cid){
        $sa = \App\Newapplicant::where('candidate_id',$cid)->first();
        $rid = $sa->randstrig;
        $applicantid = str_pad($sa->id,5,'0',STR_PAD_LEFT);
    ...
    public function downloadmssupp($cid,$term){
     ...
     public function downloadcertsupp($cid){
         $sa = \App\Supplimentaryapplicant::where('candidate_id',$cid)->first();
         $rid = $sa->randstrig;
         $applicantid = str_pad($sa->id,5,'0',STR_PAD_LEFT);
        ...
        //if($currentapplicant->approvedprogramme->programme_id != 36){
            if(!is_null($currentapplicant) && Auth::user()->id == $currentapplicant->candidate->user_id){
                ...
                return response()->download($file, $filename, $headers);
                   ...
                   // $job = (new \App\Jobs\GenerateMarksheet($currentapplicant->candidate_id,$term))->onQueue('studentpm');
                    //$this->dispatch($job);
                    Session::put('messages','Not Generated');
                    return back();
                }
