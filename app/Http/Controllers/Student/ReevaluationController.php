...
class ReevaluationController extends Controller
{
    ...
    public function index(Request $r){
      ...
      if($r->type=='reevaluation'){
            ...
            $reevaluationapplication = Reevaluationapplication::where('exam_id',27)->where('candidate_id',$candidate->id);
            if($reevaluationapplication->count() == 0){
                $count = Reevaluationapplication::where('exam_id',27)->where('nber_id',$nber_id)->count();
                $count++;
                $nber  = \App\Nber::find($nber_id)->short_code;
                $applicationnumber = '27'.$nber.'REAPPN'.str_pad($count, 4, '0', STR_PAD_LEFT);
                ...
                foreach($subjects as $subject){
                    if($r->has('reevaluation_'.$subject->id) || $r->has('retotal_'.$subject->id) ||$r->has('photocopy_'.$subject->id) ){
                        $currentapplication_id = \App\Allapplication::where('candidate_id',$candidate->id)->where('subject_id',$subject->id)->first()->id;
        ...
        $exam_id = 27;
        $candidate = Candidate::where('user_id',Auth::user()->id)->first();
        $newapplicant = \App\Allapplicant::where('candidate_id',$candidate->id)->where('exam_id',$exam_id)->whereIn('blocked', [1, 2])->first();
        if(!is_null($newapplicant) && ($newapplicant->block == 0 || $newapplicant->block == 2)){
            $reevaluationfee = Reevaluationapplicationfee::where('exam_id',$exam_id)->first();
            //return $reevaluationfee;
            $reevaluationapplication = Reevaluationapplication::where('exam_id',$exam_id)->where('candidate_id',$candidate->id)->first();
            ...
            return view('student.reevaluation.index',compact('candidate','reevaluationfee','reevaluationapplication','newapplicant'));
        }else{
            Session::put('error',"Not avbailable");
            return back();
        }
    }
    ...
    public function ccavenuePaymentGatewayRequestHandler(Request $request) {
       ...
       // $academicyear_id = \App\Academicyear::where('current',1)->first()->id;
        ...
        $data += ['redirect_url' => url("/student/reevaluation/ccavenuepaymentgatewayresponsehandler")];
        $data += ['cancel_url' => url("student/reevaluation/ccavenuepaymentgatewayresponsehandler")];
    ...
    public function ccavenuePaymentGatewayResponseHandler(Request $request) {
        ...
        //$enrolmentfee  = Enrolmentfeepayment::where('institute_id',$institute->id)->where('academicyear_id',$academicyear_id)->where('orderstatus_id','!=',1)->where('nber_id',$nber_id)->first();
        $reevaluationapplication = Reevaluationapplication::where('exam_id',$exam_id)->where('candidate_id',$candidate->id)->where('orderstatus_id','!=',1)->first();
        $reevaluationfee = Reevaluationapplicationfee::where('exam_id',$exam_id)->first();
    ...
    public function recheckStatus($nid,$oid){
        ...
        if($obj->Order_Status_Result->order_status == 'Shipped'){
            $order->order_status = "Success";
            $order->save();
            $reevaluationapplication = Reevaluationapplication::where('exam_id',27)->where('candidate_id',$candidate->id)->first();
            $reevaluationapplication->orderstatus_id = 1;
            $reevaluationapplication->order_id=$order->id;
            $reevaluationapplication->save();
    ...
    public function cancel($id){
        ...
        if($r->orderstatus_id==1){
            Session::put('messages','Already paid');
            return back();
        }
        $candidate = Candidate::where('user_id',Auth::user()->id)->first();
