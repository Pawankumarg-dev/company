...
class HomeController extends Controller
{
    ...
    public function index() {
        $title = "Exam Center Login Page";

        return view('externalexamcenter.index', compact('title'));
    }
    ...
    public function oldchecklogin(Request $request) {
        ...
        $externalexamcenter = Externalexamcenter::where('code', $request->externalexamcenter_code)
    ...
    public function oldshowExamSchedules($exc_id, $e_id) {
        $exam = $exam = Exam::where('id', $e_id)->first();
        $title = $exam->name." Attendance Sheet - Exam Schedules Page";
        $externalexamcenter = Externalexamcenter::where('id', $exc_id)->first();
        $examcenterdetails = Externalexamcenterdetail::where('exam_id', $exam->id)->where('externalexamcenter_id', $externalexamcenter->id)->where('active_status', 1)->get();
    ...
    public function checklogin(Request $request) {
        $validator = validator($request->all());
        $externalexamcenter = Externalexamcenter::where('code', strtoupper(trim($request->externalexamcenter_code)))->where('active_status', '1')->first();
        ...
        $validator->after(function ($validator) use ($request, $externalexamcenter) {
            if(is_null($request->externalexamcenter_code) || is_null($request->externalexamcenter_password)) {
                ...
                if(is_null($externalexamcenter)) {
                    ...
                    if(strtolower(trim($request->externalexamcenter_password)) != strtolower(trim($externalexamcenter->password))) {
                        $validator->errors()->add('externalexamcenter_code', 'Please enter the valid login credentials');
                    }
                }
            }
        ...
        return redirect('/externalexamcenter/showhomepage/'.$externalexamcenter->id);
    }

    public function showhomepage($exc_id) {
        $title = "Exam Center Home Page";
        $externalexamcenter = Externalexamcenter::find($exc_id);

        $exam_ids = Application::where("externalexamcenter_id", $externalexamcenter->id)->groupBy("exam_id")->pluck("exam_id")
                        ->toArray();
        $show = 0;

        /*
        $uncoded = array("WB", "MH", "OR", "MZ", "MN", "AS", "TN", "KA", "KE", "AP");
        if(in_array(substr($externalexamcenter->code, 2, 2), $uncoded)) {
            $show++;
        }
        */

        $exams = Exam::whereIn("id", $exam_ids)->where("examination_status", 1)->orderBy('sortorder')->get();

        return view('externalexamcenter.showhomepage', compact('title', 'externalexamcenter', 'exams', 'show'));
    }
    ...
    public function viewAttendanceSheets($e_id, $exc_id, $et_id) {
        ...
        if(!is_null($externalexamcenter)) {
            $applications = Application::where("applications.externalexamcenter_id", $exc_id)
            ...
            */
            ...
            $approvedprogrammes = Approvedprogramme::with('institute', 'academicyear')
                ->whereIn("approvedprogrammes.id", $approvedprogramme_ids)
                ->get()->sortBy('institute.code')->sortBy('academicyear.year');
