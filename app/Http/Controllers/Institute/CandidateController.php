...
class CandidateController extends Controller
{
        ...
        if($ap && $ap->institute_id == \App\Institute::where('user_id',Auth::user()->id)->first()->id){
           ...
           /* if($ap->academicyear->current==1 or ($ap->programme->nber_id = 3 && ($ap->academicyear_id < 11 or $ap->academicyear_id == 10))){
                ...
                if($enable == 1 && $ap->institute->user_id==Auth::user()->id){
                    ...
                    return view('institute.candidates.index1',compact('candidates','ap'));
    ...
    public function niepvdedit($id){
        ...
        $ap = Approvedprogramme::find($apid);
        if($ap->academicyear_id == 12 && $ap->enable_admission == 1 && $now->toDateString() <= \Carbon\Carbon::parse($ap->enable_admission_till)->toDateString() ){
        ...
        // if($ap && $ap->programme_id == 43 ||$ap->programme_id == 20){
                ...
                //  $disabilities = Disabilitytype::all();
                    ...
                    /*   if($ap->programme->nber_id == 2){
                        ...
                        return view('institute.candidates.edit',compact('mindate','nationalities','yesno','apid','states','candidate','salutations','cities','genders','communities','institute','programme','ap'));
                    }else{
                        if($ap->maxintake > $ap->candidates()->count()){
                            return view('institute.candidates.create',compact('mindate','nationalities','yesno','apid','states', 'salutations','cities','genders','communities','institute','programme','ap'));
                        }
                    }
                }
            //}
...
/*        }else{
    ...
    public function uploadsignature(Request $r){

        if(!file_exists(public_path().'/files/temp/'.$r->img)){
    	    return json_encode('failed');
        }
        $file = public_path().'/files/temp/'.$r->img;
        try{
            $filestart = 'signature_'.$r->candidate_id;
            $ex = explode('.', $file);
            $extn = end($ex);
            ...
            return json_encode('failedexception'); 
        }
        $candidate = Candidate::find($r->candidate_id);
        $candidate->signature = 'cropped/'.$filename;
        $candidate->save();
    ...
    private function storeorupdate($request, $candidate = null){
        ...
        $ap = Approvedprogramme::find($request->approvedprogramme_id);
        // if( $ap->programme->id != 57){
        //     return '';
        // }


    $admitted=\App\Candidate::where('approvedprogramme_id',$ap->id)->count();

        $now = \Carbon\Carbon::now();

            $otp_db=\App\EpariveshStudent::where('RegistrationNo',$request->epariveshreg)->pluck('otp');

     $sessionotp = Session::get('studentverification_otp');

     
        if($otp_db[0]!=$request->ottp)
        {
          if($sessionotp !=$request->ottp){
            Session::put('error','Kindly enter valid OTP');
            return back();
          }
        }
...

{
        ...
        Session::forget('studentverification_otp');
...
//enable validation name aadhaar,dob,father name,


        if($ap->enable_admission == 1 && $now->toDateString() <= \Carbon\Carbon::parse($ap->enable_admission_till)->toDateString()){
            
      //if(Auth::user()->id != 23153)   {return 'Closed';}
       ...
       $mindate = date('Y-m-d', $mindate); */
        ...
        if($candidate==null){
            ...
            // $rules['name'] = 'required|alpha_spaces|max:99|regex:/^[^.]*$/';
                        $rules['name'] = 'required';
...
//  $pariveshStudent =  \App\EpariveshStudent::where('RegistrationNo', $request->reg)->update(['admitted' => 1]);
            ...
            $candidatedatahistory->ip_address = $request->ip();

            $candidatedatahistory->save();
            $candidate->update($request->except('aadhar','fathername','name','photo','signature','doc_tenth','doc_application','doc_twelveth','doc_community'));
 ...
 public function student_exam_details($id)
    {
        //dd($id);
        $candidate = Candidate::find($id);
        ...
        $marks = DB::select("
            ...
            from
                allapplications 
            where candidate_id = " . $id . " and allapplications.exam_id <28
            ) as m
            left join subjects s on s.id = m.subject_id
            inner join candidates c on c.id = m.candidate_id
            inner join subjecttypes st on st.id = s.subjecttype_id
            inner join exams e on e.id = m.exam_id and publish_result=1
            order by e.id desc, s.syear, s.subjecttype_id, s.sortorder
        ");
         // dd( $marks);

        return view('institute.candidates.student_exam_details', compact('candidate', 'marks'));
    }
