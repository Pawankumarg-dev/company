...
class HallticketController extends Controller
{
        ...
        if($ap){
            if($ap->institute->user_id==Auth::user()->id){
               // $candidates = Candidate::where('approvedprogramme_id',$ap->id)->get();
                ...
                return view('institute.halltickets.index',compact('ap','exam_id','id'));
            }else{
                return redirect('/');
            }
        }
    ...
    public function attendance(Request $request){
      ...
      //  $docex  = $request->document_exemption;
        ...
        $attendance = Attendance::where('candidate_id',$request->candidate_id)->where('exam_id',$exam_id);
        if($attendance->count()>0){
            $attendance = $attendance->first();
            $attendance->update([
                'attendance_t'=>$request->attendance_t,
