...
/*
    ...
    public function verify(Request $r){
        ...
        $candidate->candidateapprovals()->create(['user_id'=>Auth::user()->id,'comment'=>'Verified the documents']);
        return back();
    }
    ...
    public function reject(Request $r){
        ...
        $candidate->save();
        Session::put('message','Updated');
        $candidate->candidateapprovals()->create(['user_id'=>Auth::user()->id,'comment'=> ' Changed status to pending, reason: '. $r->comment ]);
        return back();
    }
        ...
        if($ap){
            //if($ap->institute->user_id==Auth::user()->id){
              ...
              //  $disabilities = Disabilitytype::all();
                ...
                if($candidate){
                    return view('nber.candidates.edit',compact('mindate','nationalities','yesno','apid','states','candidate','salutations','cities','genders','communities','institute','programme'));
                }else{
    ...
    public function show($id){
      ...
      /*  if($candidate->approvedprogramme->academicyear_id == $current_exam->academicyear_id){
        ...
        $exams = \App\Exam::where('id','<',24)->orderBy('id','desc')->get();
        $marks = DB::select("
            select  
                m.id,
                e.name, 
            ...
            union 
            select grace, id, candidate_id, applicant_id, exam_id, subject_id, internalattendance_id, externalattendance_id, internal_mark,  result_id, external_mark, 0 as ra_id from applications sa
            where candidate_id = " . $id ."
        ...
        $reevaluationapplicaiton = \App\Reevaluationapplication::where('candidate_id',$candidate->id)->where('exam_id',22)->where('orderstatus_id',1)->first();
        $attendace = \App\Attendance::where('candidate_id',$candidate->id)->where('exam_id',25)->first();
        $exam_center = null;
        $fy_count = 0;
        $sy_count = 0;
        ...
        return view('nber.candidates.view_details',compact('fy_count','sy_count','exam_center','exams','attendace','supplimentaryapplicant','reevaluationapplicaiton','candidate','current_exam','marks','subjects'));
        //}else{
         //   return back();
        //}
    }
