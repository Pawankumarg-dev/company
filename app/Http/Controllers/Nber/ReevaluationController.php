...
class ReevaluationController extends Controller
{
    ...
    ->join('examcenters AS kv', function($join) {
        $join->on('kv.institute_id', '=', 'i.id')
             ->where('kv.exam_id', '=', 27); // use where for exam_id filter
    })
    ...
    ->groupBy('evc.id');
        ...
        if($show == 2){
            ...
            $total = $total->where('rs.active_status',1)->where('rs.exam_id',27);
        }
    ...
    public function index(Request $r) {
        ...
        // if($exam_id != 22 && $exam_id != 25){
        //     Session::put('error','Not available in the chooseen exam');
        //     return back();
        // }
        ...
        // }
        // if($exam_id == 25){
        //     $evaluationcenterIds = \App\Evaluationcenterdetail::where('exam_id',$exam_id)->pluck('evaluationcenter_id')->toArray();
        //     $evaluationcenters = \App\Evaluationcenter::whereIn('id',$evaluationcenterIds)->get();
        // }
         $evaluationcenterIds = \App\Evaluationcenterdetail::where('exam_id',$exam_id)->pluck('evaluationcenter_id')->toArray();
            $evaluationcenters = \App\Evaluationcenter::whereIn('id',$evaluationcenterIds)->get();
        ...
        if($r->has('search')){
            ...
            $reevaluationapplications = Reevaluationapplication::where('exam_id', $exam->id)->where('nber_id',$nber_id);
            }
        }
        if($r->has('evaluationcenter_id') && $r->evaluationcenter_id !=0 && !$r->has('search')){
            $evaluation_center = \App\Evaluationcenter::find($r->evaluationcenter_id);
                ...
                $externalexamcenter_ids = \App\Evaluationcenterdetail::where('evaluationcenter_id',$r->evaluationcenter_id)->where('exam_id',$exam->id)->pluck('externalexamcenter_id');
                $institute_id = \App\Examcenter::whereIn('externalexamcenter_id',$externalexamcenter_ids)->pluck('institute_id');
                $reevaluationapplications = $reevaluationapplications->whereHas('institute',function($q) use($institute_id){
    ...
    public function showApplications($id){
        ...
        public function recheckStatus($nid,$oid,$rid){
       // return back();
        set_time_limit(0);
        ...
        if($obj->Order_Status_Result->order_status == 'Shipped'){
            $order->order_status = "Success";
            $order->save();
            $reevaluationapplication = Reevaluationapplication::where('id',$rid)->first();
            $reevaluationapplication->orderstatus_id = 1;
            $reevaluationapplication->order_id=$order->id;
            $reevaluationapplication->save();
