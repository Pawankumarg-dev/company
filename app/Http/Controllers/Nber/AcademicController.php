...
class AcademicController extends Controller
{
    public function __construct()
    {
	 ...
	 public function index(Request $r){
        ...
        if($r->has('search')){
            $collections = Institute::whereHas('approvedprogrammes',function($q) use ($academicyear_id, $programme_ids){
                $q->where('academicyear_id',$academicyear_id)->whereIn('programme_id',$programme_ids);
            });
            $collections = $collections->where('code','like','%'.$r->search.'%')->orWhere('rci_code','like','%'.$r->search.'%');
          ...
          //  $collections = Institute::where('active_status', '1')->orderBy('code')->paginate(100);
            if($r->has('course_id')){
                $collections = Institute::whereHas('approvedprogrammes',function($q) use ($academicyear_id, $r){
                    $q->where('academicyear_id',$academicyear_id)->whereHas('programme',function($p) use ($r){
                        $p->where('course_id',$r->course_id);
                    });
                })->paginate(100);
            }else{
                $collections = Institute::whereHas('approvedprogrammes',function($q) use ($academicyear_id, $programme_ids){
        ...
        $programmes = Programme::where('active_status',1)->get();
        $kvs = Externalexamcenter::all();
        
        return view('nber.admissions.index', compact('kvs','programmes','collections','link','text','currentacademicyear','academicyear_id','enrolment','nber_id'));
    }
    public function updatekv(Request $r){
        return "Closed";
       // return 'KV list is being generated......Cannot edit now';
        if(Kvtti::where('institute_id',$r->institute_id)->count()>0){
            $maps = Kvtti::where('institute_id',$r->institute_id)->get();
            foreach($maps as $map){
                DB::table('kvttis')->where('institute_id',$r->institute_id)->update(array(
                    'externalexamcenter_id'=>$r->kv
                ));
            }
        }else{
            DB::insert('insert into kvttis(externalexamcenter_id,institute_id) values(?,?)',[$r->kv,$r->institute_id]);
        }

        return redirect('nber/admissions');
    }
    ...
    public function delete(Request $r){
        $approvedprogramme = Approvedprogramme::find($r->id);
    ...
    public function create(Request $request){
        $this->validate($request, [
            ...
            $ap = Approvedprogramme::create(['institute_id'=>$request->institute_id,'programme_id'=>$request->programme_id,'academicyear_id'=>Session::get('academicyear_id'),'status_id'=>1,'maxintake'=>$request->maxintake]);
