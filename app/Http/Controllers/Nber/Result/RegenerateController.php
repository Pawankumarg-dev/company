...
class RegenerateController extends Controller
{
    ...
    public function regenerate($caid,$term){
        ...
        $currentapplicat = \App\Currentapplicant::where('candidate_id',$caid)->first();
...
//		return response()->json(['certificate',$certificate]);
         //   return $certificate ;//== true ? 'T': 'F';
            $job = (new \App\Jobs\GenerateMarksheet($candidate_id,$term))->onQueue('march2025');
            $this->dispatch($job);
        ...
        if($certificate){
            $job = (new \App\Jobs\GenerateCertificate($candidate_id))->onQueue('march2025');
            $this->dispatch($job);
        }
        return back();
    }
