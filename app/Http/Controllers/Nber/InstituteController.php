...
class InstituteController extends Controller
{
    ...
    public function genenno($iid){
        ...
        Session::put('messages','Enrolment No generated');
        return back();
    }

    public function enrolmentfeereport(){
        $nber_id = \App\Nberstaff::where('user_id',Auth::user()->id)->first()->nber_id;
        $institutes = \App\Institute::whereHas('enrolmentfeepayments',function($q) use ($nber_id){
            $q->where('nber_id',$nber_id)->where('orderstatus_id',1);
        })->get();
       // return $institutes->count();
        //$enrolmentfees = \App\Enrolmentfeepayment::where('nber_id',$nber_id)->where('academicyear_id',Session::get('academicyear_id'))->get();
        return view('nber.enrolmentfee.report',compact('institutes','nber_id'));


    }

    public function enrolmentfee($iid){
        $nber_id = \App\Nberstaff::where('user_id',Auth::user()->id)->first()->nber_id;
        $approvedprogrammes = Approvedprogramme::where('academicyear_id',Session::get('academicyear_id'))->where('institute_id',$iid)->whereHas('programme',function($q) use($nber_id){
            $q->where('nber_id',$nber_id);
        })->get();
        
        $institute_id = $iid;
        $i = Institute::find($iid);
        $enrolmentfees = \App\Enrolmentfeepayment::where('institute_id',$iid)->where('academicyear_id',Session::get('academicyear_id'))->get();
        return view('nber.enrolmentfee.index',compact('approvedprogrammes','nber_id','institute_id','enrolmentfees','i'));
    }

    public function recheckPayment($id,$fid){
     return $this->recheckService->recheck($id,$fid);
    }

    public function searchrefno(Request $r){
        return $this->recheckService->checkwithrefno($r->refno);
    }

    public function examapplications(){
        $institutes = Institute::all();
        return view('nber.institutes.examapplications',compact('institutes'));
    }

    public function changepassword(Request $r){
        $institute = Institute::find($r->id);
        if($institute->user_id > 0){
    ...
    public function issues(Request $request){
        ...
        $nberId = \App\Nberstaff::where('user_id', Auth::user()->id)->first()->nber_id;
        //$nberId = 0;
        // $issues = \App\Reportedissue::where('nber_id',$nberId)->paginate(100);

        $query = \App\Reportedissue::query();
        $query->where('nber_id',$nberId);
        if ($request->has('status')) {
            $status = $request->get('status');
            if ($status !== 'all') {
                $query->where('solved', $status == 'solved' ? 1 : 0);
            }
        }
        $issues = $query->paginate(100); 
        ...
        SELECT
        ri.*,
            nb.name_code AS 'nber',
            COUNT(*) AS 'total_grievance',
            COUNT(CASE WHEN ri.solved = 1 THEN 1 END) AS 'grievance_complete',
            COUNT(CASE WHEN ri.solved = 0 THEN 1 END) AS 'total_grievance_pending',
            COUNT(CASE WHEN ri.solved = 0 AND ri.created_at >= CURDATE() - INTERVAL 7 DAY THEN 1 END) AS 'pending_last_7_days',
            COUNT(CASE WHEN ri.solved = 0 AND ri.created_at >= CURDATE() - INTERVAL 15 DAY AND ri.created_at < CURDATE() - INTERVAL 7 DAY THEN 1 END) AS 'pending_last_15_days',
            COUNT(CASE WHEN ri.solved = 0 AND ri.created_at >= CURDATE() - INTERVAL 30 DAY AND ri.created_at < CURDATE() - INTERVAL 15 DAY THEN 1 END) AS 'pending_last_30_days',
            COUNT(CASE WHEN ri.solved = 0 AND ri.created_at < CURDATE() - INTERVAL 30 DAY THEN 1 END) AS 'pending_more_than_30_days'
        FROM
            reportedissues ri
        INNER JOIN
            nbers nb ON ri.nber_id = nb.id where nb.id=".$nberId."
        GROUP BY
            nb.short_name_code
    ");
        return view('nber.issues.index',compact('issues','grievance'));
    }
	
    public function solveissue($iid){
        $issue = \App\Reportedissue::find($iid);
        return view('nber.issues.solve',compact('issue'));
    }

    public function solved(Request $r){
        $issue =  \App\Reportedissue::find($r->id);
        $issue->solved = 1;
        $issue->solutions = $r->solutions;
        $issue->save();
        Session::put('messages','Marked as solved');
   ...
   // 	$courseCoordinatorCount = Coursecoordinator::where('active_status', 1)->count();
    ...
    public function create(Request $request){
        ...
        $messages = [
            'username.required' => 'The Institute Code is required',
            'username.unique' => 'The Institute Code is already taken',
        ...
        $mysqluser = User::create(['username'=>$request->username,'password'=>bcrypt($request->password),'email'=>$request->email,'usertype_id'=>'2','confirmation_code'=>$request->password,'database_name'=>$database]);
        \Illuminate\Support\Facades\DB::setDefaultConnection($database);
        $user = User::create(['id' => $mysqluser->id , 'username'=>$request->username,'password'=>bcrypt($request->password),'email'=>$request->email,'usertype_id'=>'2','confirmation_code'=>$request->password]);
    ...
    public function showInstituteLists() {
        ...
        return view('nber.institutes.centreinformation.showInstituteLists', compact('institutes'));
    }
