...
class QuestionPaperController extends Controller
{
    ...
    public function __construct(ExamService $exam,HelperService $helper)
    {
     ...
     public function index(Request $r){
        ...
        $sdate = \Carbon\Carbon::parse($schedule->examdate)->toDateString();
        ...
        //|| Auth::user()->id == 134579 || ( $schedule->qpset > 0 && (  $schedule_id == 54 ) )
           ...
           // if($externalexamcenter->second_year_strenght > 0){
            ...
            /*    $sql = "select s.id as subject_id, s.has_alternative, n.name_code, n.email, p.display_code, group_concat(distinct y.year) as batches, s.scode, s.sname,  count(*) as noofstudent, tt.id as id, group_concat(distinct a.language_id) as language_ids from newapplications na
                ...
                //return $exams[0];
                 $downloadedcount = \App\Allexampaper::where('examschedule_id',$schedule_id)
                ->where('externalexamcenter_id',$externalexamcenter->id)
                ->whereNotNull('downloaded_session')->count();
                if($downloadedcount!=0){
                    $loggedinsession = \App\Allexampaper::where('examschedule_id',$schedule_id)
                    ->where('externalexamcenter_id',$externalexamcenter->id)
                ...
                if($downloadedcount == 0){
                    if($schedule->description == 'Mockdrill'){
                        ...
                        ->where('externalexamcenter_id',$externalexamcenter->id)
                        ->selectRaw('*, group_concat(course) as courses, group_concat(distinct scode) as subjectcode, 1 as batches,theory as no_of_student')
                        ->groupBy('omr_code');
                        return view('examcenter.questionpaper.demo',compact(
                            'exams',
                            'schedule',
                            'session_id'
                        ));
